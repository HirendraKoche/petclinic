pipeline{
	...
	post{
		...
		success{
                        ansiColor('xterm') {
                           ansiblePlaybook colorized: true, disableHostKeyChecking: true, extraVars: [BUILD_TAG: $BUILD_TAG], inventory: 'jenkins/docker/deploy/host', playbook: 'jenkins/docker/deploy/deploy.yml'
                        }
